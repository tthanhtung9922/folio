...
| **Ghost Ink**        | `#9A8878` | Metadata, timestamp, muted label                                                             |
...
| **Ghost Ink**        | `#7A6A5A` |
| **Terracotta**       | `#D4723D` |
| **Terracotta Pale**  | `#3A2820` |
| **Parchment Border** | `#3A3028` |

**Implementation:** `@theme inline` maps Tailwind tokens to `var(--folio-*)` CSS custom properties. The `.dark` class overrides those vars — all utility classes auto-switch without `dark:` prefixes.

**Nguyên tắc:** Terracotta là màu accent duy nhất trong cả hai mode. Palette gần như monochrome — contrast đến từ font weight và khoảng trống, không phải màu sắc.
...
- 10px terracotta circle, `mix-blend-mode: multiply` (light) / `mix-blend-mode: screen` (dark — multiply goes invisible on dark backgrounds)
