...
Thêm block `ConnectionStrings` vào (placeholder cho production — sẽ được override bằng env var khi deploy):

```json
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Port=5432;Database=folio;Username=postgres;Password=CHANGE_ME"
  },
  "Logging": {
    "LogLevel": {
      "Default": "Information",
...
> **Không commit password thật vào đây.** File `appsettings.json` được commit vào Git và chỉ chứa placeholder. Giá trị thật được inject qua environment variable hoặc Docker secret khi deploy.
...
File này ghi đè `appsettings.json` khi chạy local (Development environment). Dùng thông tin match với `infra/docker-compose.dev.yml`:

```json
{
  "ConnectionStrings": {
    "DefaultConnection": "Host=localhost;Port=5432;Database=folio_dev;Username=dev;Password=dev"
  },
  "Logging": {
    "LogLevel": {
      "Default": "Information",
...
> **`appsettings.Development.json` không bị commit vào Git không?** File này **được** commit — credentials ở đây chỉ là dev local (`dev/dev`), không phải production. Đây là convention chuẩn của .NET.
